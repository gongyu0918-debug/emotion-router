...
| `urgent-pressure-routing` | user gives immediate deadline pressure or the delay is escalating | urgent pressure + evidence request | lead with the shortest reliable basis, keep the first reply brief, and name the next update checkpoint |
| `exploratory-option-selection` | user asks to compare architectures, options, or implementation paths | exploratory option selection | provide a ranked option set with tradeoffs and do not force a single path before risk or user choice demands it |
